...
function mtScore(entry_id) {
    ...
    xh.send( '__mode=vote&blog_id=99&f=scored,count&jsonp=mtScore_cb&id=' + entry_id);
    return false;
}
...
function mtUpdateScores() {
    ...
    xh.setRequestHeader( 'Content-Type', 'application/x-www-form-urlencoded' );
    xh.send( '__mode=score&blog_id=99&f=scored,count&jsonp=mtScore_cb&id=' + entry_ids);
    return false;
}
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://www.washingtonian.com/cgi-bin/mt/mt-comments.cgi?__mode=session_js&blog_id=99&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '75bf275ef099bf7c387e59135a7bbe709be29606';
...
function mtSignIn() {
    ...
    var url = 'http://www.washingtonian.com/cgi-bin/mt/mt-cp.cgi?__mode=login&blog_id=99';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fwww.washingtonian.com%2Fquizzes%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://www.washingtonian.com/cgi-bin/mt/mt-cp.cgi?__mode=logout&blog_id=99';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://www.washingtonian.com/cgi-bin/mt/mt-cp.cgi?__mode=edit&amp;return_to=' + encodeURIComponent(document.URL) + '&amp;blog_id=99';
                user_link += '">' + u.name + '</a>';
            } else {
